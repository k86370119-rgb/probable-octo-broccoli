...
export default function App() {
  ...
  const send = async (text, i64 = null) => {
    ...
    const content = i64
      ? [
          { type: "image_url", image_url: { url: "data:image/jpeg;base64," + i64 } },
          { type: "text", text: text || "Explain this educational image step by step for a Kenyan student." }
        ]
      : text;

    const newMsgs = [...msgs, { role: "user", content: content }];
    const displayMsgs = [...msgs, { role: "user", content: text || "Photo sent for explanation", img: i64 ? img : null }];
    ...
    try {
      ...
      const res = await fetch(API, {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [
            { role: "system", content: SYSTEM + " Level:" + level + " Subject:" + subject },
            ...newMsgs
          ]
        })
      });
      ...
      setMsgs(prev => [...prev, { role: "assistant", content: "Samahani! Could not connect. Please try again! 🙏" }]);
    }
  ...
  if (screen === "home") return (
    <div style={G.page}>
      <Bars />
      <div style={G.card}>
        <h1 style={{ fontSize: 30, fontWeight: 900, color: "#006600", margin: "0 0 4px", fontFamily: "Georgia,serif" }}>
          MsomaBuddy
        </h1>
        <p style={{ color: "#BB0000", fontWeight: 700, fontSize: 14, margin: "0 0 4px" }}>
          🇰🇪 Kenya's Smart Study Partner
        </p>
        <div style={{ background: "linear-gradient(135deg,#1a1a1a,#006600)", color: "#c8960c", fontSize: 12, fontWeight: 800, padding: "4px 14px", borderRadius: 20, display: "inline-block", marginBottom: 12 }}>
          ⚡ V2 — Photo + Video + Research!
        </div>
        <p style={{ fontSize: 14, color: "#444", lineHeight: 1.6, margin: "0 0 14px" }}>
          AI help for <b>CBC</b> and <b>Secondary</b> students.<br />Strictly educational. Available 24/7!
        </p>
        ...
        <div style={{ background: "#fff3f3", border: "1.5px solid #BB0000", borderRadius: 10, padding: "8px", marginBottom: 12, fontSize: 12, color: "#BB0000", fontWeight: 700 }}>
          🚫 Strictly Education Only
        </div>
        <button style={G.btn} onClick={() => setScreen("level")}>
          Anza Kusoma! 🚀
        </button>
        <p style={{ fontSize: 11, color: "#999", marginTop: 6 }}>
          "Anza Kusoma" = Start Studying
        </p>
        <div style={{ marginTop: 20, padding: 16, borderRadius: 14, background: "linear-gradient(135deg,#1a1a1a,#006600)", border: "2px solid #c8960c", textAlign: "center" }}>
          <img
            ...
            alt="Kelsey"
            style={{ width: 70, height: 70, borderRadius: "50%", objectFit: "cover", border: "3px solid #c8960c", marginBottom: 8 }}
          />
          <p style={{ color: "#c8960c", fontWeight: 900, fontSize: 14, margin: "2px 0" }}>👩‍💻 Made by Kelsey Wangui Wanjiku</p>
          <p style={{ color: "#fff", fontSize: 11, margin: "2px 0" }}>Education and Tech Enthusiast 🎓</p>
  ...
  if (screen === "level") return (
    <div style={G.page}>
      <Bars />
      <div style={G.card}>
        <button style={{ background: "none", border: "none", color: "#006600", fontSize: 15, fontWeight: 700, cursor: "pointer", padding: "0 0 14px", fontFamily: "Georgia,serif" }} onClick={() => setScreen("home")}>
          Back
        </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            { id: "CBC", label: "CBC Student", desc: "Grade 1 to 9", e: "🏫" },
            { id: "Secondary", label: "Secondary Student", desc: "Form 1 to 4 KCSE", e: "📚" }
          ].map(l => (
            <button key={l.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "20px", borderRadius: 14, border: "2.5px solid #006600", background: "#e8f5e9", cursor: "pointer", fontFamily: "Georgia,serif" }}
              onClick={() => { setLevel(l.id); setScreen("subject"); }}>
  ...
  if (screen === "subject") return (
    <div style={G.page}>
      <Bars />
      <div style={G.card}>
        <button style={{ background: "none", border: "none", color: "#006600", fontSize: 15, fontWeight: 700, cursor: "pointer", padding: "0 0 14px", fontFamily: "Georgia,serif" }} onClick={() => setScreen("level")}>
          Back
        </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {subs.map(s => (
            <button key={s} style={G.sbj} onClick={() => {
              setSubject(s);
              setMsgs([{ role: "assistant", content: "Habari! I am MsomaBuddy, your study partner for " + s + "!\n\nUse the tabs to:\n📸 Snap a photo of a question\n🎥 Record a study video\n🔬 Research a topic\n\nOr just ask me anything! Tuanze! 💪🇰🇪" }]);
              setMode("chat");
              setScreen("chat");
            }}>{s}</button>
          ))}
  ...
  return (
    <div style={G.chat}>
      <div style={G.hdr}>
        ...
        <div>
          <div style={{ fontWeight: 800, fontSize: 15, color: "#fff", fontFamily: "Georgia,serif" }}>MsomaBuddy</div>
          <div style={{ fontSize: 10, color: "#c8ffc8" }}>{subject} · {level} · Edu Only 🔒</div>
        </div>
        <button style={{ background: "#BB0000", border: "none", color: "#fff", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
          onClick={() => { setMsgs([]); setScreen("subject"); }}>New</button>
      </div>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "2px solid #006600" }}>
        {[{ id: "chat", l: "💬 Chat" }, { id: "photo", l: "📸 Photo" }, { id: "video", l: "🎥 Video" }, { id: "research", l: "🔬 Research" }].map(m => (
          <button key={m.id} style={{ ...G.tab, ...(mode === m.id ? { color: "#006600", borderBottom: "3px solid #006600", background: "#e8f5e9" } : {}) }}
            onClick={() => setMode(m.id)}>{m.l}</button>
        ))}
      </div>
      ...
      {mode === "photo" && (
        <div style={{ background: "#fff", padding: 14, borderBottom: "1px solid #eee" }}>
          ...
          {img && (
            <button style={{ ...G.btn, fontSize: 14, padding: "11px" }} onClick={() => { setMode("chat"); send("", img64); }}>
              🧠 Explain This!
            </button>
          )}
      ...
      {mode === "video" && (
        <div style={{ background: "#fff", padding: 14, borderBottom: "1px solid #eee" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            ...
            <button style={G.mdb} onClick={() => vidRef.current.click()}>📁 Upload Video</button>
          </div>
          <input ref={vidRef} type="file" accept="video/*" capture="environment" style={{ display: "none" }} onChange={() => alert("Video saved as study material! Use Chat to ask questions.")} />
      ...
      {mode === "research" && (
        <div style={{ background: "#fff", padding: 14, borderBottom: "1px solid #eee" }}>
          ...
          <textarea
            style={{ width: "100%", padding: "10px", borderRadius: 10, border: "2px solid #006600", fontSize: 13, fontFamily: "Georgia,serif", background: "#e8f5e9", outline: "none", resize: "none", boxSizing: "border-box" }}
          ...
          />
          <button
            style={{ ...G.btn, fontSize: 14, padding: "11px" }}
            onClick={() => {
              if (!research.trim()) return;
              const p = "Research topic: " + research + ". Give a detailed curriculum summary for " + level + " " + subject + " in Kenya. Include key concepts, examples and exam tips.";
              setResearch("");
              setMode("chat");
              send(p);
            }}
      ...
      <div style={G.body}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 10 }}>
            <div style={m.role === "user" ? G.usr : G.ai}>
              {m.img && <img src={m.img} alt="upload" style={{ maxWidth: "100%", borderRadius: 8, marginBottom: 6 }} />}
              {(typeof m.content === "string" ? m.content : "📸 Photo sent").split("\n").map((l, j) => (
        ...
        {loading && (
          <div style={{ display: "flex", gap: 5, padding: "10px 0" }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ width: 9, height: 9, borderRadius: "50%", background: "#006600", animation: "b 1.2s " + i * 0.2 + "s infinite" }} />
            ))}
            <style>{`@keyframes b{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-8px)}}`}</style>
          </div>
        )}
        {error && (
          <div style={{ background: "#fff0f0", border: "1px solid #BB0000", borderRadius: 10, padding: "8px 12px", fontSize: 12, color: "#BB0000", margin: "8px 0" }}>
            ⚠️ {error}
          </div>
        )}
        <div ref={ref} />
      ...
      {msgs.length <= 1 && mode === "chat" && (
        <div style={{ display: "flex", gap: 8, padding: "8px 12px", overflowX: "auto", background: "#fff", borderTop: "1px solid #eee" }}>
          {[
            { l: "📖 Explain", t: "Explain main concepts in " + subject },
            { l: "🧪 Quiz Me", t: "Give me a quiz on " + subject },
            { l: "💡 Tip", t: "Give me a study tip for " + subject },
            { l: "📝 Past Paper", t: "Give me a past paper question for " + subject + " in Kenya" }
          ].map(a => (
            <button key={a.l} style={{ whiteSpace: "nowrap", padding: "8px 12px", borderRadius: 20, border: "2px solid #BB0000", background: "#fff0f0", color: "#BB0000", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Georgia,serif" }}
              onClick={() => send(a.t)}>{a.l}</button>
          ))}
        </div>
      )}
